...
import { getSrvFuncName } from '../common/utils'
    ...
    function walk(entry: string, type: ExportType) {
        if (type instanceof ExportFunc) {
            const [srvName, funcName] = getSrvFuncName(entry),
                requestType = `${srvName}_${funcName}_kyk_req`,
                responseType = `${srvName}_${funcName}_kyk_res`,
                srv = nested[srvName] || (nested[srvName] = { methods: { } })
            srv.methods[funcName] = { requestType, responseType, ...type.opts }
